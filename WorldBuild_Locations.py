...
import random
import math
from PoissonDiskGeneratorIntegers import pds
...
#        destination = str(r'c:\\picts\\%d'% step) + str(r'.png')
        ...
        obj = pds( 128, 128, 6, 10 )
        sample1 = obj.rvs()
        x1 = [int(p[0]) for p in sample1]
        y1 = [int(q[1]) for q in sample1]
        
        print "number of points in x1 is %d", len(x1)
        print "x1 is %d", x1
        
        print "number of points in y1 is %d", len(y1)
        print "y1 is %d", y1
        
        x2 = []
        y2 = []
        
        for aa in range(0, len(x1)):
            #print self.__DisplayMap[x1][y1]
            if (self.__DisplayMap[x1[aa]][y1[aa]] > 0):
                #print x1[a], y1[a]
                x2.append(x1[aa])
                y2.append(y1[aa])
        ...
        print "number of points in y2 is %d", len(x2)
        print "x2 is %d", x2
        
        print "number of points in y2 is %d", len(y2)
        print "y2 is %d", y2
        
        fig, ax = plt.subplots()
        ax.imshow(self.__DisplayMap, interpolation='none', origin='upper', cmap=cm.winter)
        CS = ax.contour(self.__DisplayMap, [0, 1], cmap=cm.winter)
        CB = plt.colorbar(CS, shrink=0.8, extend='both')
        l,b,w,h = plt.gca().get_position().bounds
        ll,bb,ww,hh = CB.ax.get_position().bounds
        CB.ax.set_position([ll, b+0.1*h, ww, h*0.8])
        
        ax.scatter(y1, x1, color='red', marker='s')
        #ax.scatter(y2, x2, color='yellow', marker='s')
